...
public class LoginPage extends LoadableComponent<LoginPage> {
	...
	protected void isLoaded() throws Error {
		//Assert.assertTrue(Web.getDriver().getCurrentUrl().contains(CommonLib.getSponser()),"Login Page is Not Loaded");
		try {
...

		{
			if (Stock.GetParameterValue("AccuCode") != null)
				...
				accuCode = CommonLib.GC_DEFAULT_SPONSER;
			...
			Assert.assertTrue(
					...
					"Login page for '" + CommonLib.getSponser()
							+ "' is not loaded.");
	...
	protected void load() {
		...
		// lib.Web.getDriver().get(Stock.getConfigParam("AppURL"));
			...
			url = Stock.getConfigParam("AppURLPPTWEB");
   ...
   for(int i=0;i<contactNo.size();i++)
   {
	...
	public void verifyLinkIsNotBroken(String linkName)
		...
		if (isPageDisplayed) {
			Reporter.logEvent(Status.PASS, "Verify " + linkName
					+ " Page is Displayed", linkName + " Page is Displayed",
					true);

		} else {
			Reporter.logEvent(Status.FAIL, "Verify " + linkName
					+ " Page is Displayed",
	...
	public List<String> getPreLoginTelePhoneNo(String sponsorName) {
		...
		/*if (sponsorName.equalsIgnoreCase(CommonLib.GC_DEFAULT_SPONSER)) {
	...
	/*public boolean verifyAccuCodeisSame()
		...
		try {
			...
			if (Stock.globalTestdata.get(Thread.currentThread().getId())
				...
				accucodeFromTestData = CommonLib.GC_DEFAULT_SPONSER;
			}
