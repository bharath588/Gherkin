...
public class validate_loanquotepage {
	...
	public void Validate_Message_for_Selected_Loan_and_Request_Loan_Quote(int itr,Map<String, String> testdata) {
		...
		try {
			...
			if(loanQuotePO.VerifyMsg_for_Selected_and_Existing_Loan(Stock.GetParameterValue("MsgType"))){
				Reporter.logEvent(Status.PASS,"Validate if "+Stock.GetParameterValue("MsgType").toUpperCase()
						  +" loan type message is displayed correctly",
						  Stock.GetParameterValue("MsgType").toUpperCase()+" loan type message is displayed successfully",false);
			}else{
				Reporter.logEvent(Status.FAIL,"Validate if "+Stock.GetParameterValue("MsgType").toUpperCase()
				          +" loan type message is displayed correctly",
				          Stock.GetParameterValue("MsgType").toUpperCase()+" loan type message is displayed inaccurate",true);
			}			
		} catch (Exception e) {
			e.printStackTrace();
			Globals.exception = e;
	 ...
	 */
	@Test(dataProvider = "setData")
	public void Validate_Selected_Loan_Type_Details(int itr,Map<String, String> testdata) {
		loanQuotePO = new LoanQuote() ;
		...
		try {
		    ...
		    // Step2:Navigating to Loan Quote 
				loanQuotePO.get();
				loanQuotePO.SelectLoanType(Stock.GetParameterValue("LoanType"));
				if(!Web.isWebElementDisplayed(loanQuotePO,"BTN_CALC_AVAIL_LOAN_AMT",true)){
					Reporter.logEvent(Status.FAIL,"Navigation to Loan Quote page unsuccessfull",
				    		  "Loan Quote page didn't get displayed",true);
				}else{
					Reporter.logEvent(Status.INFO,"Navigation to Loan Quote page is successfull",
			...
			if(loanQuotePO.Verify_Details_for_Selected_Loan(infoType)){
				Reporter.logEvent(Status.PASS,"Validate "+infoType+" details",infoType+" details successfully validated" ,false);
			}else{
				Reporter.logEvent(Status.FAIL,"Validate "+infoType+" details",infoType+" details validation failed",true);
			}			
		} catch (Exception e) {
			e.printStackTrace();
			Globals.exception = e;
