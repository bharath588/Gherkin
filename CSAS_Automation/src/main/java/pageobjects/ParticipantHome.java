...
public class ParticipantHome extends LoadableComponent<ParticipantHome> {
	...
	private void initTempandOrderPINMsg(){
		Mail_Existing_PIN.put("MSG1","Combined verification and pin letter generated: doc_id = "
		...
		Order_Temp_PIN.put("MSG1","Note: this VRU PIN (or web passcode) is a single use PIN (passcode) "
		...
		Mail_Existing_VRUPIN_and_Web_PassCode.put("MSG1","An order to mail the participant's existing web passcode "
				+ "has been issued, confirmation #.");
		Mail_Existing_VRUPIN_and_Web_PassCode.put("MSG2","An order to mail the participant's existing VRU PIN "
				+ "has been issued, confirmation #.");
		
		Mail_Existing_Web_Passcode.put("MSG1","An order to mail the participant's existing web passcode "
				+ "has been issued, confirmation #.");
		
		Mail_Existing_VRUPIN.put("MSG1","An order to mail the participant's existing VRU PIN "
				+ "has been issued, confirmation #.");
		
		Order_Temp_VRU_PIN_and_Web_PassCode.put("MSG1","Note: this VRU PIN (or web passcode) is a single use PIN (passcode) that must "
				+ "be re-set after first use. "
				+ "Please direct participant accordingly.");
		Order_Temp_VRU_PIN_and_Web_PassCode.put("MSG2","The temporary web passcode number is");
		Order_Temp_VRU_PIN_and_Web_PassCode.put("MSG3","The temporary VRU PIN number is");
		
		Order_Temp_Web_PassCode.put("MSG1","Note: this VRU PIN (or web passcode) is a single use PIN (passcode) that must "
				+ "be re-set after first use. "
				+ "Please direct participant accordingly.");
		Order_Temp_Web_PassCode.put("MSG2","The temporary web passcode number is");
		
		Order_Temp_VRUPIN.put("MSG1","Note: this VRU PIN (or web passcode) is a single use PIN (passcode) that must "
				+ "be re-set after first use. "
				+ "Please direct participant accordingly.");
	...
	public Map<String,String> getSSN_or_pptID(String WebRegStatus,String... paramNm) throws Exception {
	 ...
	 * @throws Exception
	 */
	public void search_PPT_Plan_With_PPT_ID_OR_SSN(String ssnOrIDElement,String... searchValue) throws Exception {
	...
	public void verifyPIN_ExistingOrTemp() throws Throwable {
		...
		getMessage = (Map<String, String>) getVarByName(Stock.GetParameterValue("btnName"));
	...
	public ArrayList<String> get_Participant_ID_From_DB_For_Managed_Acc_Status(
			String[] get_IndID_PlanID_FromPartService, String managed_Acc_Status) throws Exception {
	 ...
	 * @throws Exception 
	 */
	public String get_Plan_number_From_DB(String[] get_Plan_Num_From_PartService) throws Exception {
		ResultSet resultset;
	 ...
	 */
	public void click_And_Verify_Plan_On_Search_Page(String plan_Number_DB) throws InterruptedException {
		...
		if (Web.isWebElementDisplayed(partList_Tab)) {
			...
			for (int i = 0; i <= PlanNoOnPartList_Link.size()-1; i++) {
				...
				if (plan_Number_DB.equalsIgnoreCase(plan_No_Web) 
					&& partList_Tab.findElement(By.xpath("//tr["+(i+1)+"]/td[16]"))
					   .getText().equalsIgnoreCase("ACTIVE")) {
					
					partList_Tab.findElement(By.xpath("//tr["+(i+1)+"]/td[1]/a")).click();
					Web.waitForElement(PPTHomePageTitle);
					...
					if (isElementDisplayed) {
						...
						Reporter.logEvent(
								Status.FAIL,
								"Participant Home Page should display",
								"Participant Home Page is not displayed",
								true);
					}
					break;
				}
