...
public class IORN_InquireRemittance {
	
	public String queryString;
	private String serviceURL =  "http://" + Stock.getConfigParam("TargetServiceHostServer").trim() + ":8080/ServiceManager/Macro/ExecMacro/IORN";
	...
	public String getRMNC_ID_0() {
		return RMNC_ID_0;
	}
	public void setRMNC_ID_0(String rMNC_ID_0) {
		RMNC_ID_0 = rMNC_ID_0;
	}
	...
	public void setServiceParameters()
	{
		...
		this.setRMNC_ID_0(Stock.GetParameterValue("RMNC_ID_0"));
		this.setLOGON_DATABASE(Stock.GetParameterValue("LOGON_DATABASE"));
		this.setLOGON_USERNAME(Stock.GetParameterValue("LOGON_USERNAME"));
		this.setLOGON_PASSWORD(Stock.GetParameterValue("LOGON_PASSWORD"));
		queryString = "?LOGON_USERNAME=" + LOGON_USERNAME + "&LOGON_PASSWORD=" + LOGON_PASSWORD + "&LOGON_DATABASE=" + LOGON_DATABASE
				+"&CONTROL_SELECTION_0=" + CONTROL_SELECTION_0 + "&RMNC_ID_0=" + RMNC_ID_0
				+"&numOfRowsInTable=20&json=false&handlePopups=false";	
		Reporter.logEvent(Status.INFO, "Prepare test data for IORN service", this.queryString.replaceAll("&", "\n"), false);
	}
	...
	public void runService() {
		try {
			...
			Reporter.logEvent(Status.PASS, "Run IORN service", "Execution Done!", false);
		} catch (Exception e) {
			e.printStackTrace();
			Globals.exception = e;
			Reporter.logEvent(Status.FAIL, "Run IORN service", "Running Failed:", false);
		}
	}
...
//			Reporter.logEvent(Status.PASS, "Capturing Response after Execution", "Response:\n"+doc.getDocumentElement(), false);
		...
		/*String ev_id = null;
		queryResultSet = DB.executeQuery(General.dbInstance, Stock.getTestQuery("queryForINDR")[1], this.RMNC_ID_0);
		...
		if(this.RMNC_ID_0.equalsIgnoreCase(ev_id)){
			Reporter.logEvent(Status.PASS, "Comparing and Validating  ID from Input and Database", "Both the values are same as Expected"+
					"\nID: "+"From Input: "+this.RMNC_ID_0+"\nFrom Database: "+ev_id, false);
