...
public class GVAG_Query_GROUP_VARIABLE_ANNUITY_COMP_STRUC {
	
	public String queryString;
	 private String serviceURL =  "http://" + Stock.getConfigParam("TargetServiceHostServer").trim() + ":8080/ServiceManager/Macro/ExecMacro/GVAG_Query_GROUP_VARIABLE_ANNUITY_COMP_STRUC";
		...
		public void setServiceParameters()
		{
			...
			+ LOGON_DATABASE +"&CONTROL_SELECTION_0=" + CONTROL_SELECTION_0 + ""
					+ "&CONTROL_SELECTION_0_X1=" + CONTROL_SELECTION_0_X1 +"&GROUP_ACCOUNT_ID_0=" + GROUP_ACCOUNT_ID_0 +
		...
		public void runService() {
			try {
				...
				Reporter.logEvent(Status.PASS, "Run GVAG service", "Execution Done!", false);
			} catch (Exception e) {
				e.printStackTrace();
				Globals.exception = e;
				Reporter.logEvent(Status.FAIL, "Run GVAG service", "Running Failed:", false);
			}
		}
		...
		public void validateInDatabase() throws SQLException{
			String dbId= null;
			queryResultSet = DB.executeQuery(General.dbInstance, Stock.getTestQuery("queryForGVAG")[1]);
			if(DB.getRecordSetCount(queryResultSet)>0){
				Reporter.logEvent(Status.PASS, "Validating Records exists in Database", "Records exists in Database", false);
	            int i=0;
				while(queryResultSet.next()){
				
					dbId = queryResultSet.getString("ID_2");
					String formId = doc.getElementsByTagName("EXT_GVA_COMP_STRUC_EGFA_ID_0").item(i).getTextContent();
					if(dbId.equalsIgnoreCase(formId))
					{
						Reporter.logEvent(Status.PASS,"Validating if the values from the database matches the form values.",
								"Values from the database matches the form values.Database value:"+dbId+"\n Value from form"+formId,false);	
						...
						else
						{
							Reporter.logEvent(Status.FAIL,"Validating if the values from the database matches the form values",
									"Values from the database does not match the form values.Database value:"+dbId+"\n Value from form"+formId,false);
