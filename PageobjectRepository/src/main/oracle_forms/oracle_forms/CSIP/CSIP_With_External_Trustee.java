...
public class CSIP_With_External_Trustee {

	public String queryString;
	private String serviceURL =  "http://" + Stock.getConfigParam("TargetServiceHostServer").trim() + ":8080/ServiceManager/Macro/ExecMacro/CSIP_with_External_Trustee";
	...
	public void setServiceParameters(String remit_id, String amount)	
	{
		this.setLOGON_DATABASE(Stock.GetParameterValue("LOGON_DATABASE"));
		this.setLOGON_PASSWORD(Stock.GetParameterValue("LOGON_PASSWORD"));
		this.setLOGON_USERNAME(Stock.GetParameterValue("LOGON_USERNAME"));
		this.setCONTROL_SELECTION_0(Stock.GetParameterValue("CONTROL_SELECTION_0"));
		this.setAG_REMIT_AMOUNT_0(amount);
	...
	public void validateInDatabase()throws SQLException{
		...
		String status_code_agremit = null;
		String status_code_indremit = null;
		
		queryResultSet = DB.executeQuery(General.dbInstance, Stock.getTestQuery("queryForCSIPExtTrustee1")[1], rmnc_id);
		if(DB.getRecordSetCount(queryResultSet)>0){
			...
			while(queryResultSet.next()){				
				status_code_agremit = queryResultSet.getString("PROCESS_STATUS_CODE");
		...
		queryResultSet = DB.executeQuery(General.dbInstance, Stock.getTestQuery("queryForCSIPExtTrustee2")[1], rmnc_id);
		if(DB.getRecordSetCount(queryResultSet)>0){
			Reporter.logEvent(Status.PASS, "Validating existence of data in the Database", "Record exists in the Database\nTable Name: AG_REMIT", false);
			while(queryResultSet.next()){				
				status_code_indremit = queryResultSet.getString("PROCESS_STATUS_CODE");
				Reporter.logEvent(Status.INFO, "Values from Database", "RMNC_ID: "+queryResultSet.getString("RMNC_ID")+					
		...
		if(status_code_agremit.equalsIgnoreCase("D") && (status_code_indremit.equalsIgnoreCase("D"))){
			Reporter.logEvent(Status.PASS, "Comparing Status Code from AG_REMIT and IND_REMIT tables equals to \"D\" ", "Status codes in both tables are equal to \"D\" "+
					"From AG_REMIT: "+status_code_agremit+"From IND_REMIT: "+status_code_indremit, false);
		}else{
			Reporter.logEvent(Status.FAIL, "Comparing Status Code from AG_REMIT and IND_REMIT tables equals to \"D\" ", "Status codes in both table are not equal to \"D\" "+
					"From AG_REMIT: "+status_code_agremit+"From IND_REMIT: "+status_code_indremit, false);
		}
