 ...
 *
 */
public class CSIP_LARGE_DOLLAR_AG_REMIT_2 {
	public String queryString;
	private String serviceURL =  "http://" + Stock.getConfigParam("TargetServiceHostServer").trim() + ":8080/ServiceManager/Macro/ExecMacro/CSIP_LARGE_DOLLAR_AG_REMIT_2";
	...
	public void setServiceParameters(String remit_id)	
	{
		...
		this.setCONTROL_SELECTION_0(Stock.GetParameterValue("CONTROL_SELECTION_CSIP"));
		...
		Reporter.logEvent(Status.INFO, "Prepare test data for CSIP_LARGE_DOLLAR_AG_REMIT_2 service", this.queryString.replaceAll("&", "\n"), false);
	}
	...
	public void runService() {
		try {
			...
			Reporter.logEvent(Status.PASS, "Run CSIP_LARGE_DOLLAR_AG_REMIT_2 service", "Execution Done!", false);
		} catch (Exception e) {
			e.printStackTrace();
			Globals.exception = e;
			Reporter.logEvent(Status.FAIL, "Run CSIP_LARGE_DOLLAR_AG_REMIT_2 service", "Running Failed:", false);
		}
	}
	...
	public void validateInDatabase()throws SQLException{
		...
		int dbCount = DB.getRecordSetCount(queryResultSet);
		if(dbCount==1){
			Reporter.logEvent(Status.PASS, "Single record is created in AG_REMIT table.", "Record is created in AG_REMIT table.\n"
					+ "Record count in DB="+dbCount, false);
			while(queryResultSet.next()){
				amount = queryResultSet.getString("AMOUNT");
			}
			if(amount.equals(AG_REMIT_AMOUNT_0))
				Reporter.logEvent(Status.PASS,"Validate count from DB.","From DB:\namount="+amount+"\nExpected="+AG_REMIT_AMOUNT_0,false);
			else
				Reporter.logEvent(Status.FAIL,"Validate count from DB.","From DB:\namount="+amount+"\nExpected="+AG_REMIT_AMOUNT_0,false);
		}else{
			Reporter.logEvent(Status.FAIL, "Single record is created in AG_REMIT table.", "Record is not created in AG_REMIT table.\n"
					+"Record count in DB="+dbCount, false);
		}
	}
}
