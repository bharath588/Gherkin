...
public class CSIP_with_Positive_Balance {

	public String queryString;
	private String serviceURL =  "http://" + Stock.getConfigParam("TargetServiceHostServer").trim() + ":8080/ServiceManager/Macro/ExecMacro/CSIP_REMIT_POSITIVE_BALANCE";
	...
	public void validateInDatabase()throws SQLException{
		...
		queryResultSet = DB.executeQuery(General.dbInstance, Stock.getTestQuery("queryForCSIPRemitPositiveBal1")[1], rmnc_id);
		if(DB.getRecordSetCount(queryResultSet)>0){
			...
			while(queryResultSet.next()){
				Reporter.logEvent(Status.INFO, "From Database ", "RMNC_ID: "+queryResultSet.getString("RMNC_ID")+
						...
						"\nPAYROLL_DATE: "+queryResultSet.getString("PAYROLL_DATE"), false);
			}
		}else{
			Reporter.logEvent(Status.FAIL, "Validating existence of data in the Database", "Record doesn't exists in the Database", false);
		}
		
		queryResultSet = DB.executeQuery(General.dbInstance, Stock.getTestQuery("queryForCSIPRemitPositiveBal2")[1], rmnc_id);
		if(DB.getRecordSetCount(queryResultSet)>0){
