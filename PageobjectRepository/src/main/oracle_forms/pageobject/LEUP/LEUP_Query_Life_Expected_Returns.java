...
public class LEUP_Query_Life_Expected_Returns {

	public String queryString;
	private String serviceURL =  "http://" + Stock.getConfigParam("TargetServiceHostServer").trim() + ":8080/ServiceManager/Macro/ExecMacro/LEUP_Query_Life_Expected_Returns";
	...
	public void validateInDatabase()throws SQLException{
		...
		queryResultSet = DB.executeQuery(General.dbInstance, Stock.getTestQuery("queryForLEUPQuery1")[1]);
		if(DB.getRecordSetCount(queryResultSet)>0){
			Reporter.logEvent(Status.PASS, "Validating existence of data in the Database", "Record exists in the Database", false);
			
			Reporter.logEvent(Status.INFO, "Values from Database\nTable Name: IRS_LIFE_EXP_V", "Expected:\nUser is able to query One Life (V Table) & Expected Returns. "+
		...
		queryResultSet = DB.executeQuery(General.dbInstance, Stock.getTestQuery("queryForLEUPQuery2")[1]);
		if(DB.getRecordSetCount(queryResultSet)>0){
			Reporter.logEvent(Status.PASS, "Validating existence of data in the Database", "Record exists in the Database", false);
			
			Reporter.logEvent(Status.INFO, "Values from Database\nTable Name: IRS_LIFE_EXP_VI", "Expected:\nUser is able to query Two Lives (VI table) for all Ages & Expected Returns. "+
			"Records/Templates exists\nTotal number of records: "+DB.getRecordSetCount(queryResultSet), false);
		}
		else{
			Reporter.logEvent(Status.FAIL, "Validating existence of data in the Database", "No Record exists in the Database", false);
