...
public class GQ19VWPR_QYVWPR_Query_Participants {
	
	public String queryString;
	private String serviceURL = "http://"
			+ Stock.getConfigParam("TargetServiceHostServer").trim()
			+ ":8080/ServiceManager/Macro/ExecMacro/GQ19VWPR_QYVWPR_Query_Participants";
	...
	public void validateInDatabase() throws SQLException {
		...
		queryResultSet = DB.executeQuery(General.dbInstance,
				Stock.getTestQuery("queryForVWPR2")[1],this.PRODUCER_ACCT_NBR_0);
		...
		queryResultSet2 = DB.executeQuery(General.dbInstance,
				Stock.getTestQuery("queryForVWPR3")[1]);
		
		if (DB.getRecordSetCount(queryResultSet2) > 0){
			Reporter.logEvent(Status.PASS,
					"Validating Records exists in Database",
					"Records exists in Database", false);
			System.out.println("The count of the records fetched from the are:"+DB.getRecordSetCount(queryResultSet2));
			Reporter.logEvent(Status.INFO,"Validating the number of rows fetched.","Number of rows fetched is:"+DB.getRecordSetCount(queryResultSet2), false);
		}
		else {
			Reporter.logEvent(
					Status.FAIL,
		...
		queryResultSet3 =  DB.executeQuery(General.dbInstance,
				Stock.getTestQuery("queryForVWPR4")[1]);
		if (DB.getRecordSetCount(queryResultSet3) > 0){
			Reporter.logEvent(Status.PASS,
					"Validating Records exists in Database",
					"Records exists in Database", false);
			System.out.println("The count of the records fetched from the are:"+DB.getRecordSetCount(queryResultSet3));
			Reporter.logEvent(Status.INFO,"Validating the number of rows fetched.","Number of rows fetched is:"+DB.getRecordSetCount(queryResultSet3), false);
		}
		else {
			Reporter.logEvent(
					Status.FAIL,
