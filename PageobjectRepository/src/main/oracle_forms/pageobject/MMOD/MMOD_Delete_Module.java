...
public class MMOD_Delete_Module {

	public String queryString;
	private String serviceURL =  "http://" + Stock.getConfigParam("TargetServiceHostServer").trim() + ":8080/ServiceManager/Macro/ExecMacro/MMOD_Delete_Module";
	...
	public void validateInDatabase() throws SQLException{
		...
		queryResultSet = DB.executeQuery(General.dbInstance, Stock.getTestQuery("queryForMMODDel")[1], this.MO1_SHORT_NAME_0);
		if(DB.getRecordSetCount(queryResultSet) == 0){
			Reporter.logEvent(Status.PASS, "Validating record deleted in DB", "Records deleted in DB\nTableName: MODULE", false);
		}else{
			Reporter.logEvent(Status.FAIL, "Validating record deleted in DB", "Records deleted in DB\nTableName: MODULE", false);
		}
	}
}
