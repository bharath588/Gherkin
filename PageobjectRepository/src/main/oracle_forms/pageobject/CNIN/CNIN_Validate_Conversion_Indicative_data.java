...
public class CNIN_Validate_Conversion_Indicative_data {
	...
	public void setServiceParameters()	throws SQLException
	{
		...
		queryString = "?LOGON_USERNAME="+LOGON_USERNAME+"&LOGON_PASSWORD="+LOGON_PASSWORD+"&LOGON_DATABASE="+LOGON_DATABASE+"&CONTROL_SELECTION_0="+CONTROL_SELECTION_0+
				"&CONVERSION_GROUP_ACCOUNT_SUBJECT_ID_0="+CONVERSION_GROUP_ACCOUNT_SUBJECT_ID_0+"&numOfRowsInTable=20&json=false&handlePopups=false&useLabelsAsKeys=false";
		
		Reporter.logEvent(Status.INFO, "Prepare test data for CNIN service", this.queryString.replaceAll("&", "\n"), false);
...
//			System.out.println(serviceURL);
			...
			Reporter.logEvent(Status.PASS, "Run CNIN service", "Execution Done!", false);
		} catch (Exception e) {
			e.printStackTrace();
			Globals.exception = e;
			Reporter.logEvent(Status.FAIL, "Run CNIN service", "Running Failed:", false);
		}
	}
	...
	public void validateInDatabase() throws SQLException{
		...
		String ssn_response = doc.getElementsByTagName("WORK_CONV_PART_SSN_0").item(0).getTextContent();
		
		queryResultSet = DB.executeQuery(General.dbInstance, Stock.getTestQuery("queryForCNIN")[1],  event_id, ssn_response );
		...
		if(ssn_response.equalsIgnoreCase(ssn_db)){
			Reporter.logEvent(Status.PASS, "Comparing and Validating SSN from Response and Database", "Both the values are same as Expected"+
					"\nSSN: "+"From Response: "+doc.getElementsByTagName("WORK_CONV_PART_SSN_0").item(0).getTextContent()+"\nFrom Database: "+queryResultSet.getString("SSN"), false);
		}
	}
