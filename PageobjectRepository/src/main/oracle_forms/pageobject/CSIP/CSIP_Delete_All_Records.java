 ...
 *
 */
public class CSIP_Delete_All_Records {
	public String queryString;
	private String serviceURL =  "http://" + Stock.getConfigParam("TargetServiceHostServer").trim() + ":8080/ServiceManager/Macro/ExecMacro/CSIP_Delete_All_Records";
	...
	public void setServiceParameters(String remit_id, String payroll_date)	
	{
		...
		this.setAG_REMIT_PAYROLL_DATE_0(payroll_date);
		...
		Reporter.logEvent(Status.INFO, "Prepare test data for CSIP_Delete_All_Records service",
	...
	public void runService() {
		try {
			...
			Reporter.logEvent(Status.PASS, "Run CSIP_Delete_All_Records service", "Execution Done!", false);
		} catch (Exception e) {
			e.printStackTrace();
			Globals.exception = e;
			Reporter.logEvent(Status.FAIL, "Run CSIP_Delete_All_Records service", "Running Failed:", false);
		}
	}
	...
	public void validateInDatabase()throws SQLException{
		...
		if(DB.getRecordSetCount(queryResultSet)==0){
			Reporter.logEvent(Status.PASS, "Validate  individual records have been deleted from IND_REMIT table for remittance id.", ""
					+ "Records are not found for remittance id="+REMIT_NOTICE_ID_0+" in the the Database\nTable Name: IND_REMIT", false);
		}else{
			Reporter.logEvent(Status.FAIL, "Validate  individual records have been deleted from IND_REMIT table for remittance id.", ""
					+ "Records are not deleted for remittance id="+REMIT_NOTICE_ID_0+" in the the Database\nTable Name: IND_REMIT", false);
		}
	}
