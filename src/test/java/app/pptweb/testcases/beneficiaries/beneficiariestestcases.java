...
//		}
			...
			if(lib.Web.isWebElementDisplayed(beneficiary, "DOB Error Msg"))
				error_msg=beneficiary.readErrorMessage("DOB");
			...
			lib.Web.setTextToTextBox("DOB",beneficiary,Stock.GetParameterValue("Valid_Date"));	
...
//		}
			...
			String alert_msg= beneficiary.readErrorMessage("Alert Msgt");
			if(lib.Web.VerifyText(Stock.GetParameterValue("Alert_message"),alert_msg,true))
				Reporter.logEvent(Status.PASS, "verify No Spouse Alert Message", "No Spouse Alert Message is displayed", false);
			else
...
//		}
			...
			lib.Web.clickOnElement(beneficiary, "View Beneficiary Button");
			lib.Web.waitForElement(beneficiary, "MyBeneficiaries");
			if(beneficiary.clickOnBeneficiaryFromTable(Stock.GetParameterValue("BeneficiaryName"), "Primary"))
				 Reporter.logEvent(Status.FAIL, "Verify if beneficiary present in My Beneficiaries Table", "Beneficiary present in the table",false);
			else
				 Reporter.logEvent(Status.FAIL, "Verify if beneficiary present in My Beneficiaries Table", "Beneficiary not present in the table", true);
			
//			lib.Web.setTextToTextBox("TIN",beneficiary,"764647");	
			beneficiary.enterEntityDetails();
			String error_msg = null;
			if(lib.Web.isWebElementDisplayed(beneficiary, "TIN Error Msg"))
				error_msg=beneficiary.readErrorMessage("TIN");
			if(lib.Web.VerifyText("Tax Identification number must be 9 digits.", error_msg, true))
				Reporter.logEvent(Status.PASS, "verify TIN must be 9 didgits", "Error messgae displayed : "+error_msg, false);
			else
				Reporter.logEvent(Status.FAIL, "verify TIN must be 9 didgits", "Error messgae displayed : "+error_msg,true);
	...
	public void Beneficiary_TC014_UnMarried_Trust_warning_message(int itr, Map<String, String> testdata){
		Stock.globalTestdata = testdata;
//      Globals.GBL_CurrentIterationNumber = itr;
...
//	public void Beneficiary_TC016_QJSA_with_Beneficiary_not_on_file(int itr, Map<String, String> testdata){
//		Stock.globalTestdata = testdata;
////      Globals.GBL_CurrentIterationNumber = itr;
//		
//		
...
//				else
//					Reporter.logEvent(Status.FAIL, "verify Error message displayed for QJSA account is matching", "Error Message is matching", true);
//			}
//			else	
//				Reporter.logEvent(Status.FAIL, "verify Error message displayed ", "Error Message is not displayed", true);
//			
//			if(beneficiary.ifElementDisabled("Married"))
//				Reporter.logEvent(Status.PASS, "verify Married Yes button is disabled", "Married Yes button is disabled", false);
//			else
//				Reporter.logEvent(Status.FAIL, "verify Married Yes button is disabled", "Married Yes button is not disabled", true);
//			if(beneficiary.ifElementDisabled("Unmarried"))
//				Reporter.logEvent(Status.PASS, "verify Married No button is disabled", "Married No button is disabled", false);
//			else
//				Reporter.logEvent(Status.FAIL, "verify Married No button is disabled", "Married No button is not disabled", true);
...
//			if(Stock.GetParameterValue("Beneficiary_added").equalsIgnoreCase("Yes")){
//				//check if all the text boxes are disabled
//			}
...
//		}
			...
			if(lib.Web.isWebElementDisplayed(beneficiary, "Delete button"))
			...
			lib.Web.clickOnElement(beneficiary, "Delete Checkbox");
			lib.Web.clickOnElement(beneficiary, "Delete button");
			lib.Web.clickOnElement(beneficiary, "Submit button");
			...
			if(lib.Web.isWebElementDisplayed(beneficiary, "Delete Beneficiary Error Message")){
				if(lib.Web.VerifyText(Stock.GetParameterValue("Error_message"), error_msg, true))
					...
					Reporter.logEvent(Status.FAIL, "Verify if  Error message displayed is matching", "error message is matching", true);
			}
			else
