...
public class DeferralsMainte extends LoadableComponent<DeferralsMainte> {
...
//			@FindBy(xpath = ".//*[@id='splitContribution']/.//span[2]") private WebElement txtViewOnly;
			...
			@FindBy(xpath="//div[contains(text(),'Carry over my contribution')]") private WebElement lblChainingText;
		 ...
		 * @param parent
		 */
		public DeferralsMainte(LoadableComponent<?> parent) {
			this.parent = parent;			
			PageFactory.initElements(lib.Web.getDriver(), this);
		}
		...
		protected void isLoaded() throws Error {
			...
			if (userFromDatasheet.equalsIgnoreCase(userLogedIn))*/if(Common.verifyLoggedInUserIsSame()) {
				...
				if (!lib.Web.isWebElementDisplayed(lblMyContributions,DeferralsMainte.waitforLoad)) {
					DeferralsMainte.waitforLoad = true;
					throw new Error("'My contributions' page is not loaded");
				}else{
					DeferralsMainte.waitforLoad = false;
				}
			} else {
