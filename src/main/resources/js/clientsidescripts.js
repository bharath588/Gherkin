...
/**
 * Find elements by angular attribute and atrribute value.
 *
 * @param {string} angularAttribute The angular attribute e.g. ng-click.
 * @param {string} attributeValue The angular attribute value e.g. uploadDocument().
 * @param {Element} using The scope of the search.
 *
 * @return {Array.<Element>} An array of matching elements.
 */
functions.findByAngularAttribute = function(angularAttribute, attributeValue, using) {
	using = using || document;
	var prefixes = ['ng-','ng_','data-ng-','x-ng-'];
	for (var p = 0; p < prefixes.length; ++p){
		
		var selector = '[' + prefixes[p] +' "'+angularAttribute+'"="'+attributeValue+'"]';
