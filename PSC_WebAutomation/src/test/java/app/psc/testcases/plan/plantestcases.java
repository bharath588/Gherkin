...
public class plantestcases {
	 ...
	 */
	@Test(dataProvider = "setData")
	public void TC17_Add_PPT_And_SearchForPPT(int itr,
					...
					if (employeesearch.fillEligibilityInfoPositiveFlow()) {
						if (employeesearch.enterIncomeData()) {
							...
							if (employeesearch.isSearchResultsDisplayed()) {
								Reporter.logEvent(
										Status.PASS,
										"Check if search results are displayed for ssn:"
												+ ssn,
								...
								Reporter.logEvent(
										...
										"Search results are not displayed correctly",
										true);
							}
	 ...
	 */
	@Test(dataProvider = "setData")
	public void TC18_SIT_PSC_Investment_UnitShare_values_page(int itr,
			Map<String, String> testDat) {
		try {
			Reporter.initializeReportForTC(itr, Globals.GC_MANUAL_TC_NAME);
			Reporter.logEvent(
	...
	public void TC20_Complex_User(int itr, Map<String, String> testDat) {
		...
		try {
			 ...
			 * "Navigate to Plan-->Administration-->User security management page."
			 * , "" + "user is navigated to user security management page.",
			 * false); else Reporter.logEvent(Status.FAIL,
			 * "Navigate to Plan-->Administration-->User security management page."
			 * , "" + "user is not navigated to user security management page.",
			 * true);
			 */
			queryResultSet = DB.executeQuery(
					Stock.getTestQuery("getComplexUser")[0],
					Stock.getTestQuery("getComplexUser")[1]);
			while (queryResultSet.next()) {
				complexUserAssignedId = queryResultSet.getString("LOGON_ID")
						.replace("K_", "").trim();
				break;
			}
			if (planPage.searchUser(new String[] { complexUserAssignedId },
				...
				Reporter.logEvent(Status.PASS,
						"Search for a complex user with assigned user id-"
								+ complexUserAssignedId, "" + "user is found.",
						false);
			else
				Reporter.logEvent(Status.FAIL,
						"Search for a complex user with assigned user id-"
								+ complexUserAssignedId, ""
								+ "user is not found.", true);

			if (!planPage.canBeEdited())
				Reporter.logEvent(
						Status.PASS,
						"Validate N/A is displayed for complex user against action column.",
			...
			if (planPage
					.searchUser(new String[] { Stock
							.GetParameterValue("NonComplexUserId") }, Web
							.returnElement(planPage, "ASSIGNED_USERID")))
				Reporter.logEvent(Status.PASS,
						"Search for a non complex user with assigned user id-"
								+ Stock.GetParameterValue("NonComplexUserId"),
						"" + "user is found.", false);
			else
				Reporter.logEvent(Status.FAIL,
						"Search for a non complex user with assigned user id-"
			...
			if (planPage.canBeEdited()) {
				if (planPage.validateActionDropDownOption("View / Edit"))
					Reporter.logEvent(
							Status.PASS,
							"Validate non complex user can be updated through SAW.",
				...
				Reporter.logEvent(
						...
						"Validate non complex user can be updated through SAW.",
						""
								+ "Action drop down is not displayed hence user can not be updated.",
						true);
			}
		...
		try {
			...
			if (Stock.GetParameterValue("Flow").equals("Positive")) {
				...
				if (planPage.searchUser(new String[] { lastname },
					...
					Reporter.logEvent(Status.PASS,
							"Search a user with filter last name-" + lastname,
							"" + "User is found.", false);
				else
					Reporter.logEvent(Status.FAIL,
							"Search a user with filter last name-" + lastname,
				...
				if (!planPage.searchUser(new String[] { lastname, firstname },
					...
					if (CommonLib.VerifyText(
						...
						Reporter.logEvent(
								Status.PASS,
								"Validate proper validation message is displayed.",
								"Validation message is displayed properly as below.\n"
										+ actErrorMsg, false);
					else
						Reporter.logEvent(
								Status.FAIL,
								"Validate if proper validation message is displayed.",
								"Validation message is not displayed properly.Actual message is displayed as below-\n"
										+ actErrorMsg, true);
				} else {
					Reporter.logEvent(
							Status.FAIL,
							"Search a user with invalid filters as below\nlast name-"
									+ lastname + " and first name-" + firstname,
							"" + "User is found.", true);
				}
	 ...
	 */
	@Test(dataProvider = "setData")
	public void TC25_Investment_Document_Page(int itr,
			Map<String, String> testDat) {
		try {
			Reporter.initializeReportForTC(itr, Globals.GC_MANUAL_TC_NAME);
		...
		try {
			...
			if (Stock.GetParameterValue("Terminate").equalsIgnoreCase("Yes")) {
				try {
					...
					CommonLib.switchToFrame(Web.returnElement(new LoginPage(),
							"LOGIN FRAME"));
					String actError = Web.returnElement(new LoginPage(),
							"PreLoginErrorMsg").getText();
					if (Web.VerifyText(expError, actError, true))
						Reporter.logEvent(Status.PASS,
								"Login with terminated user and validate below error message.\n"
										+ expError, ""
					...
					DB.executeUpdate(
							Stock.getTestQuery("updateTermDateToNull")[0],
